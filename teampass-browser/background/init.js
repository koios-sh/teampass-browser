...
browser.tabs.onCreated.addListener((tab) => {
    if (tab.id > 0) {
        ...
        if (tab.selected) {
            ...
            tpEvent.invoke(page.switchTab, null, tab.id, []);
        }
    }
});
    ...
    browser.tabs.get(activeInfo.tabId).then((info) => {
        if (info && info.id) {
            ...
            if (info.status === 'complete') {
                //console.log('tpEvent.invoke(page.switchTab, null, '+info.id + ', []);');
                tpEvent.invoke(page.switchTab, null, info.id, []);
            }
        }
    });
});
...
browser.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
    ...
    if (changeInfo.status === 'complete') {
        ...
        tpEvent.invoke(browserAction.removeRememberPopup, null, tabId, []);
    }
});

browser.runtime.onMessage.addListener(tpEvent.onMessage);
